/* DONT TOUCH: GENERATED FILE! */

#define NUM_ARCH_EVENTS_HASWELL 276

static PerfmonEvent  haswell_arch_events[NUM_ARCH_EVENTS_HASWELL] = {
 {"TEMP_CORE",
   "TMP0", 
   0x00,0x00,0,0}
, {"PWR_PKG_ENERGY",
   "PWR0", 
   0x00,0x00,0,0}
, {"PWR_PP0_ENERGY",
   "PWR1", 
   0x00,0x00,0,0}
, {"PWR_DRAM_ENERGY",
   "PWR3", 
   0x00,0x00,0,0}
, {"INSTR_RETIRED_ANY",
   "FIXC0", 
   0x00,0x00,0,0}
, {"CPU_CLK_UNHALTED_CORE",
   "FIXC1", 
   0x00,0x00,0,0}
, {"CPU_CLK_UNHALTED_REF",
   "FIXC2", 
   0x00,0x00,0,0}
, {"LD_BLOCKS_STORE_FORWARD",
   "PMC", 
   0x03,0x02,0,0}
, {"MISALIGN_MEM_REF_LOADS",
   "PMC", 
   0x05,0x01,0,0}
, {"MISALIGN_MEM_REF_STORES",
   "PMC", 
   0x05,0x02,0,0}
, {"MISALIGN_MEM_REF_ANY",
   "PMC", 
   0x05,0x03,0,0}
, {"LD_BLOCKS_PARTIAL_ADDRESS_ALIAS",
   "PMC", 
   0x07,0x01,0,0}
, {"DTLB_LOAD_MISSES_CAUSES_A_WALK",
   "PMC", 
   0x08,0x01,0,0}
, {"DTLB_LOAD_MISSES_WALK_COMPLETED_4K",
   "PMC", 
   0x08,0x02,0,0}
, {"DTLB_LOAD_MISSES_WALK_COMPLETED_LARGE",
   "PMC", 
   0x08,0x04,0,0}
, {"DTLB_LOAD_MISSES_WALK_COMPLETED",
   "PMC", 
   0x08,0x0E,0,0}
, {"DTLB_LOAD_MISSES_WALK_DURATION",
   "PMC", 
   0x08,0x10,0,0}
, {"DTLB_LOAD_MISSES_STLB_HIT_4K",
   "PMC", 
   0x08,0x20,0,0}
, {"DTLB_LOAD_MISSES_STLB_HIT_2M",
   "PMC", 
   0x08,0x40,0,0}
, {"DTLB_LOAD_MISSES_STLB_HIT",
   "PMC", 
   0x08,0x60,0,0}
, {"DTLB_LOAD_MISSES_PDE_CACHE_MISS",
   "PMC", 
   0x08,0x80,0,0}
, {"UOPS_ISSUED_ANY",
   "PMC", 
   0x0D,0x03,0,0}
, {"UOPS_ISSUED_ANY",
   "PMC", 
   0x0E,0x01,0,0}
, {"UOPS_ISSUED_FLAGS_MERGE",
   "PMC", 
   0x0E,0x10,0,0}
, {"UOPS_ISSUED_SLOW_LEA",
   "PMC", 
   0x0E,0x20,0,0}
, {"UOPS_ISSUED_SINGLE_MUL",
   "PMC", 
   0x0E,0x40,0,0}
, {"L2_RQSTS_ALL_DEMAND_DATA_RD_MISS",
   "PMC", 
   0x24,0x21,0,0}
, {"L2_RQSTS_ALL_DEMAND_DATA_RD_HIT",
   "PMC", 
   0x24,0x41,0,0}
, {"L2_RQSTS_ALL_DEMAND_DATA_RD",
   "PMC", 
   0x24,0xE1,0,0}
, {"L2_RQSTS_RFO_HIT",
   "PMC", 
   0x24,0x42,0,0}
, {"L2_RQSTS_RFO_MISS",
   "PMC", 
   0x24,0x22,0,0}
, {"L2_RQSTS_RFO_ANY",
   "PMC", 
   0x24,0xE2,0,0}
, {"L2_RQSTS_CODE_RD_HIT",
   "PMC", 
   0x24,0x44,0,0}
, {"L2_RQSTS_CODE_RD_MISS",
   "PMC", 
   0x24,0x24,0,0}
, {"L2_RQSTS_ALL_DEMAND_MISS",
   "PMC", 
   0x24,0x27,0,0}
, {"L2_RQSTS_ALL_DEMAND_REFERENCES",
   "PMC", 
   0x24,0xE7,0,0}
, {"L2_RQSTS_ALL_CODE_RD",
   "PMC", 
   0x24,0xE4,0,0}
, {"L2_RQSTS_L2_PF_HIT",
   "PMC", 
   0x24,0x50,0,0}
, {"L2_RQSTS_L2_PF_MISS",
   "PMC", 
   0x24,0x30,0,0}
, {"L2_RQSTS_ALL_PF",
   "PMC", 
   0x24,0xF8,0,0}
, {"L2_RQSTS_MISS",
   "PMC", 
   0x24,0x3F,0,0}
, {"L2_RQSTS_REFERENCES",
   "PMC", 
   0x24,0xFF,0,0}
, {"L2_DEMAND_RQST_WB_HIT",
   "PMC", 
   0x27,0x50,0,0}
, {"LONGEST_LAT_CACHE_REFERENCE",
   "PMC", 
   0x2E,0x4F,0,0}
, {"LONGEST_LAT_CACHE_MISS",
   "PMC", 
   0x2E,0x41,0,0}
, {"CPU_CLOCK_UNHALTED_THREAD_P",
   "PMC", 
   0x3C,0x00,0,0}
, {"CPU_CLOCK_UNHALTED_REF_P",
   "PMC", 
   0x3C,0x01,0,0}
, {"L1D_PEND_MISS_PENDING",
   "PMC1", 
   0x48,0x01,0,0}
, {"DTLB_STORE_MISSES_MISS_CAUSES_A_WALK",
   "PMC", 
   0x49,0x01,0,0}
, {"DTLB_STORE_MISSES_WALK_COMPLETED_4K",
   "PMC", 
   0x49,0x02,0,0}
, {"DTLB_STORE_MISSES_WALK_COMPLETED_LARGE",
   "PMC", 
   0x49,0x04,0,0}
, {"DTLB_STORE_MISSES_WALK_COMPLETED",
   "PMC", 
   0x49,0x0E,0,0}
, {"DTLB_STORE_MISSES_WALK_DURATION",
   "PMC", 
   0x49,0x10,0,0}
, {"DTLB_STORE_MISSES_STLB_HIT_4K",
   "PMC", 
   0x49,0x20,0,0}
, {"DTLB_STORE_MISSES_STLB_HIT_LARGE",
   "PMC", 
   0x49,0x40,0,0}
, {"DTLB_STORE_MISSES_STLB_HIT",
   "PMC", 
   0x49,0x60,0,0}
, {"DTLB_STORE_MISSES_PDE_CACHE_MISS",
   "PMC", 
   0x49,0x80,0,0}
, {"LOAD_HIT_PRE_SW_PF",
   "PMC", 
   0x4C,0x01,0,0}
, {"LOAD_HIT_PRE_HW_PF",
   "PMC", 
   0x4C,0x02,0,0}
, {"L1D_REPLACEMENT",
   "PMC", 
   0x51,0x01,0,0}
, {"MOVE_ELIMINATION_INT_NOT_ELIMINATED",
   "PMC", 
   0x58,0x04,0,0}
, {"MOVE_ELIMINATION_SIMD_NOT_ELIMINATED",
   "PMC", 
   0x58,0x08,0,0}
, {"MOVE_ELIMINATION_INT_ELIMINATED",
   "PMC", 
   0x58,0x01,0,0}
, {"MOVE_ELIMINATION_SIMD_ELIMINATED",
   "PMC", 
   0x58,0x02,0,0}
, {"CPL_CYCLES_RING0",
   "PMC", 
   0x5C,0x01,0,0}
, {"CPL_CYCLES_RING123",
   "PMC", 
   0x5C,0x02,0,0}
, {"RS_EVENTS_EMPTY_CYCLES",
   "PMC", 
   0x5E,0x01,0,0}
, {"OFFCORE_REQUESTS_OUTSTANDING_DEMAND_DATA_RD",
   "PMC", 
   0x60,0x01,0,0}
, {"OFFCORE_REQUESTS_OUTSTANDING_DEMAND_CODE_RD",
   "PMC", 
   0x60,0x02,0,0}
, {"OFFCORE_REQUESTS_OUTSTANDING_DEMAND_RFO",
   "PMC", 
   0x60,0x04,0,0}
, {"OFFCORE_REQUESTS_OUTSTANDING_ALL_DATA_RD",
   "PMC", 
   0x60,0x08,0,0}
, {"CACHE_LOCK_CYCLES_SPLIT_LOCK_UC_LOCK_DURATION",
   "PMC", 
   0x63,0x01,0,0}
, {"CACHE_LOCK_CYCLES_CACHE_LOCK_DURATION",
   "PMC", 
   0x63,0x02,0,0}
, {"IDQ_EMPTY",
   "PMC", 
   0x79,0x02,0,0}
, {"IDQ_MITE_UOPS",
   "PMC", 
   0x79,0x04,0,0}
, {"IDQ_DSB_UOPS",
   "PMC", 
   0x79,0x08,0,0}
, {"IDQ_MS_DSB_UOPS",
   "PMC", 
   0x79,0x10,0,0}
, {"IDQ_MS_MITE_UOPS",
   "PMC", 
   0x79,0x20,0,0}
, {"IDQ_MS_UOPS",
   "PMC", 
   0x79,0x30,0,0}
, {"IDQ_ALL_DSB_CYCLES_ANY_UOPS",
   "PMC", 
   0x79,0x18,0x00,0x01}
, {"IDQ_ALL_DSB_CYCLES_4_UOPS",
   "PMC", 
   0x79,0x18,0x00,0x04}
, {"IDQ_ALL_MITE_CYCLES_4_UOPS",
   "PMC", 
   0x79,0x24,0x00,0x01}
, {"IDQ_ALL_MITE_CYCLES_4_UOPS",
   "PMC", 
   0x79,0x24,0x00,0x04}
, {"IDQ_ALL_MITE_ALL_UOPS",
   "PMC", 
   0x79,0x3C,0,0}
, {"ICACHE_MISSES",
   "PMC", 
   0x80,0x02,0,0}
, {"ITLB_MISSES_CAUSES_A_WALK",
   "PMC", 
   0x85,0x01,0,0}
, {"ITLB_MISSES_WALK_COMPLETED_4K",
   "PMC", 
   0x85,0x02,0,0}
, {"ITLB_MISSES_WALK_COMPLETED_LARGE",
   "PMC", 
   0x85,0x04,0,0}
, {"ITLB_MISSES_WALK_COMPLETED",
   "PMC", 
   0x85,0x0E,0,0}
, {"ITLB_MISSES_WALK_DURATION",
   "PMC", 
   0x85,0x10,0,0}
, {"ITLB_MISSES_STLB_HIT_4K",
   "PMC", 
   0x85,0x20,0,0}
, {"ITLB_MISSES_STLB_HIT_2M",
   "PMC", 
   0x85,0x40,0,0}
, {"ITLB_MISSES_STLB_HIT",
   "PMC", 
   0x85,0x60,0,0}
, {"ILD_STALL_LCP",
   "PMC", 
   0x87,0x01,0,0}
, {"ILD_STALL_IQ_FULL",
   "PMC", 
   0x87,0x04,0,0}
, {"BR_INST_EXEC_COND_TAKEN",
   "PMC", 
   0x88,0x81,0,0}
, {"BR_INST_EXEC_COND_NON_TAKEN",
   "PMC", 
   0x88,0x41,0,0}
, {"BR_INST_EXEC_DIRECT_JMP_TAKEN",
   "PMC", 
   0x88,0x82,0,0}
, {"BR_INST_EXEC_DIRECT_JMP_NON_TAKEN",
   "PMC", 
   0x88,0x42,0,0}
, {"BR_INST_EXEC_INDIRECT_JMP_NON_CALL_RET_TAKEN",
   "PMC", 
   0x88,0x84,0,0}
, {"BR_INST_EXEC_INDIRECT_JMP_NON_CALL_RET_NON_TAKEN",
   "PMC", 
   0x88,0x44,0,0}
, {"BR_INST_EXEC_RETURN_NEAR_TAKEN",
   "PMC", 
   0x88,0x88,0,0}
, {"BR_INST_EXEC_RETURN_NEAR_NON_TAKEN",
   "PMC", 
   0x88,0x48,0,0}
, {"BR_INST_EXEC_DIRECT_NEAR_CALL_TAKEN",
   "PMC", 
   0x88,0x90,0,0}
, {"BR_INST_EXEC_DIRECT_NEAR_CALL_NON_TAKEN",
   "PMC", 
   0x88,0x50,0,0}
, {"BR_INST_EXEC_INDIRECT_NEAR_CALL_TAKEN",
   "PMC", 
   0x88,0xA0,0,0}
, {"BR_INST_EXEC_INDIRECT_NEAR_CALL_NON_TAKEN",
   "PMC", 
   0x88,0x60,0,0}
, {"BR_INST_EXEC_ALL_BRANCHES",
   "PMC", 
   0x88,0xFF,0,0}
, {"BR_MISP_EXEC_COND_TAKEN",
   "PMC", 
   0x89,0x81,0,0}
, {"BR_MISP_EXEC_COND_NON_TAKEN",
   "PMC", 
   0x89,0x41,0,0}
, {"BR_MISP_EXEC_INDIRECT_JMP_NON_CALL_RET_TAKEN",
   "PMC", 
   0x89,0x84,0,0}
, {"BR_MISP_EXEC_INDIRECT_JMP_NON_CALL_RET_NON_TAKEN",
   "PMC", 
   0x89,0x44,0,0}
, {"BR_MISP_EXEC_RETURN_NEAR_TAKEN",
   "PMC", 
   0x89,0x88,0,0}
, {"BR_MISP_EXEC_RETURN_NEAR_NON_TAKEN",
   "PMC", 
   0x89,0x48,0,0}
, {"BR_MISP_EXEC_DIRECT_NEAR_CALL_TAKEN",
   "PMC", 
   0x89,0x90,0,0}
, {"BR_MISP_EXEC_DIRECT_NEAR_CALL_NON_TAKEN",
   "PMC", 
   0x89,0x50,0,0}
, {"BR_MISP_EXEC_INDIRECT_NEAR_CALL_TAKEN",
   "PMC", 
   0x89,0xA0,0,0}
, {"BR_MISP_EXEC_INDIRECT_NEAR_CALL_NON_TAKEN",
   "PMC", 
   0x89,0x60,0,0}
, {"BR_MISP_EXEC_ALL_BRANCHES",
   "PMC", 
   0x89,0xFF,0,0}
, {"IDQ_UOPS_NOT_DELIVERED_CORE",
   "PMC", 
   0x9C,0x01,0,0}
, {"UOPS_EXECUTED_PORT_PORT_0",
   "PMC", 
   0xA1,0x01,0,0}
, {"UOPS_EXECUTED_PORT_PORT_1",
   "PMC", 
   0xA1,0x02,0,0}
, {"UOPS_EXECUTED_PORT_PORT_2",
   "PMC", 
   0xA1,0x04,0,0}
, {"UOPS_EXECUTED_PORT_PORT_3",
   "PMC", 
   0xA1,0x08,0,0}
, {"UOPS_EXECUTED_PORT_PORT_4",
   "PMC", 
   0xA1,0x10,0,0}
, {"UOPS_EXECUTED_PORT_PORT_5",
   "PMC", 
   0xA1,0x20,0,0}
, {"UOPS_EXECUTED_PORT_PORT_6",
   "PMC", 
   0xA1,0x40,0,0}
, {"UOPS_EXECUTED_PORT_PORT_7",
   "PMC", 
   0xA1,0x80,0,0}
, {"RESOURCE_STALLS_ANY",
   "PMC", 
   0xA2,0x01,0,0}
, {"RESOURCE_STALLS_RS",
   "PMC", 
   0xA2,0x04,0,0}
, {"RESOURCE_STALLS_SB",
   "PMC", 
   0xA2,0x08,0,0}
, {"RESOURCE_STALLS_ROB",
   "PMC", 
   0xA2,0x10,0,0}
, {"CYCLE_ACTIVITY_CYCLES_L2_PENDING",
   "PMC", 
   0xA3,0x01,0,0}
, {"CYCLE_ACTIVITY_CYCLES_LDM_PENDING",
   "PMC", 
   0xA3,0x02,0,0}
, {"CYCLE_ACTIVITY_STALL_L2_PENDING",
   "PMC", 
   0xA3,0x05,0,0}
, {"CYCLE_ACTIVITY_L1D_PENDING",
   "PMC", 
   0xA3,0x08,0,0}
, {"LSD_UOPS",
   "PMC", 
   0xA8,0x01,0,0}
, {"ITLB_ITLB_FLUSH",
   "PMC", 
   0xAE,0x01,0,0}
, {"OFFCORE_REQUESTS_DEMAND_DATA_RD",
   "PMC", 
   0xB0,0x01,0,0}
, {"OFFCORE_REQUESTS_DEMAND_CODE_RD",
   "PMC", 
   0xB0,0x02,0,0}
, {"OFFCORE_REQUESTS_DEMAND_RFO",
   "PMC", 
   0xB0,0x04,0,0}
, {"OFFCORE_REQUESTS_ALL_DATA_RD",
   "PMC", 
   0xB0,0x08,0,0}
, {"UOPS_EXECUTED_CORE",
   "PMC", 
   0xB1,0x02,0,0}
, {"PAGE_WALKER_LOADS_DTLB_L1",
   "PMC", 
   0xBC,0x11,0,0}
, {"PAGE_WALKER_LOADS_ITLB_L1",
   "PMC", 
   0xBC,0x21,0,0}
, {"PAGE_WALKER_LOADS_DTLB_L2",
   "PMC", 
   0xBC,0x12,0,0}
, {"PAGE_WALKER_LOADS_ITLB_L2",
   "PMC", 
   0xBC,0x22,0,0}
, {"PAGE_WALKER_LOADS_DTLB_L3",
   "PMC", 
   0xBC,0x14,0,0}
, {"PAGE_WALKER_LOADS_ITLB_L3",
   "PMC", 
   0xBC,0x24,0,0}
, {"PAGE_WALKER_LOADS_DTLB_MEMORY",
   "PMC", 
   0xBC,0x18,0,0}
, {"PAGE_WALKER_LOADS_ITLB_MEMORY",
   "PMC", 
   0xBC,0x28,0,0}
, {"TLB_FLUSH_DTLB_THREAD",
   "PMC", 
   0xBD,0x01,0,0}
, {"TLB_FLUSH_STLB_ANY",
   "PMC", 
   0xBD,0x20,0,0}
, {"INST_RETIRED_ANY_P",
   "PMC1", 
   0xC0,0x00,0,0}
, {"INST_RETIRED_ALL",
   "PMC1", 
   0xC0,0x01,0,0}
, {"OTHER_ASSISTS_AVX_TO_SSE",
   "PMC", 
   0xC1,0x08,0,0}
, {"OTHER_ASSISTS_SSE_TO_AVX",
   "PMC", 
   0xC1,0x10,0,0}
, {"OTHER_ASSISTS_ANY_WB_ASSIST",
   "PMC", 
   0xC1,0x40,0,0}
, {"UOPS_RETIRED_ALL",
   "PMC", 
   0xC2,0x01,0,0}
, {"UOPS_RETIRED_RETIRE_SLOTS",
   "PMC", 
   0xC2,0x02,0,0}
, {"MACHINE_CLEARS_MEMORY_ORDERING",
   "PMC", 
   0xC3,0x02,0,0}
, {"MACHINE_CLEARS_SMC",
   "PMC", 
   0xC3,0x04,0,0}
, {"MACHINE_CLEARS_MASKMOV",
   "PMC", 
   0xC3,0x20,0,0}
, {"BR_INST_RETIRED_ALL_BRANCHES",
   "PMC", 
   0xC4,0x00,0,0}
, {"BR_INST_RETIRED_CONDITIONAL",
   "PMC", 
   0xC4,0x01,0,0}
, {"BR_INST_RETIRED_NEAR_CALL",
   "PMC", 
   0xC4,0x02,0,0}
, {"BR_INST_RETIRED_ALL_BRANCHES",
   "PMC", 
   0xC4,0x04,0,0}
, {"BR_INST_RETIRED_NEAR_RETURN",
   "PMC", 
   0xC4,0x08,0,0}
, {"BR_INST_RETIRED_NOT_TAKEN",
   "PMC", 
   0xC4,0x10,0,0}
, {"BR_INST_RETIRED_NEAR_TAKEN",
   "PMC", 
   0xC4,0x20,0,0}
, {"BR_INST_RETIRED_FAR_BRANCH",
   "PMC", 
   0xC4,0x40,0,0}
, {"BR_MISP_RETIRED_ALL_BRANCHES",
   "PMC", 
   0xC5,0x00,0,0}
, {"BR_MISP_RETIRED_CONDITIONAL",
   "PMC", 
   0xC5,0x01,0,0}
, {"BR_MISP_RETIRED_ALL_BRANCHES",
   "PMC", 
   0xC5,0x04,0,0}
, {"BR_MISP_RETIRED_NOT_TAKEN",
   "PMC", 
   0xC5,0x10,0,0}
, {"BR_MISP_RETIRED_TAKEN",
   "PMC", 
   0xC5,0x20,0,0}
, {"FP_ASSIST_X87_OUTPUT",
   "PMC", 
   0xCA,0x02,0,0}
, {"FP_ASSIST_X87_INPUT",
   "PMC", 
   0xCA,0x04,0,0}
, {"FP_ASSIST_SIMD_OUTPUT",
   "PMC", 
   0xCA,0x08,0,0}
, {"FP_ASSIST_SIMD_INPUT",
   "PMC", 
   0xCA,0x10,0,0}
, {"FP_ASSIST_ANY",
   "PMC", 
   0xCA,0x1E,0,0}
, {"ROB_MISC_EVENT_LBR_INSERTS",
   "PMC", 
   0xCC,0x20,0,0}
, {"MEM_TRANS_RETIRED_LOAD_LAT",
   "PMC", 
   0xCD,0x01,0,0}
, {"MEM_UOP_RETIRED_LOADS",
   "PMC", 
   0xD0,0x81,0,0}
, {"MEM_UOP_RETIRED_STORES",
   "PMC", 
   0xD0,0x82,0,0}
, {"MEM_UOP_RETIRED_LOADS_STLB_MISS",
   "PMC", 
   0xD0,0x11,0,0}
, {"MEM_UOP_RETIRED_STORES_STLB_MISS",
   "PMC", 
   0xD0,0x12,0,0}
, {"MEM_UOP_RETIRED_LOADS_LOCK",
   "PMC", 
   0xD0,0x21,0,0}
, {"MEM_UOP_RETIRED_STORES_LOCK",
   "PMC", 
   0xD0,0x22,0,0}
, {"MEM_UOP_RETIRED_LOADS_SPLIT",
   "PMC", 
   0xD0,0x41,0,0}
, {"MEM_UOP_RETIRED_STORES_SPLIT",
   "PMC", 
   0xD0,0x42,0,0}
, {"MEMLOAD_UOPS_RETIRED_L1_HIT",
   "PMC", 
   0xD1,0x01,0,0}
, {"MEMLOAD_UOPS_RETIRED_L2_HIT",
   "PMC", 
   0xD1,0x02,0,0}
, {"MEMLOAD_UOPS_RETIRED_LLC_HIT",
   "PMC", 
   0xD1,0x04,0,0}
, {"MEMLOAD_UOPS_RETIRED_L2_MISS",
   "PMC", 
   0xD1,0x10,0,0}
, {"MEMLOAD_UOPS_RETIRED_HIT_LFB",
   "PMC", 
   0xD1,0x40,0,0}
, {"MEM_LOAD_UOPS_LLC_HIT_RETIRED_XSNP_MISS",
   "PMC", 
   0xD2,0x01,0,0}
, {"MEM_LOAD_UOPS_LLC_HIT_RETIRED_XSNP_HIT",
   "PMC", 
   0xD2,0x02,0,0}
, {"MEM_LOAD_UOPS_LLC_HIT_RETIRED_XSNP_HITM",
   "PMC", 
   0xD2,0x04,0,0}
, {"MEM_LOAD_UOPS_LLC_HIT_RETIRED_XSNP_NONE",
   "PMC", 
   0xD2,0x08,0,0}
, {"MEM_LOAD_UOPS_LLC_MISS_RETIRED_LOCAL_DRAM",
   "PMC", 
   0xD3,0x01,0,0}
, {"BACLEARS_ANY",
   "PMC", 
   0xE6,0x1F,0,0}
, {"L2_TRANS_DEMAND_DATA_RD",
   "PMC", 
   0xF0,0x01,0,0}
, {"L2_TRANS_RFO",
   "PMC", 
   0xF0,0x02,0,0}
, {"L2_TRANS_CODE_RD",
   "PMC", 
   0xF0,0x04,0,0}
, {"L2_TRANS_ALL_PREF",
   "PMC", 
   0xF0,0x08,0,0}
, {"L2_TRANS_L1D_WB",
   "PMC", 
   0xF0,0x10,0,0}
, {"L2_TRANS_L2_FILL",
   "PMC", 
   0xF0,0x20,0,0}
, {"L2_TRANS_L2_WB",
   "PMC", 
   0xF0,0x40,0,0}
, {"L2_TRANS_ALL_REQUESTS",
   "PMC", 
   0xF0,0x80,0,0}
, {"L2_LINES_IN_I",
   "PMC", 
   0xF1,0x01,0,0}
, {"L2_LINES_IN_S",
   "PMC", 
   0xF1,0x02,0,0}
, {"L2_LINES_IN_E",
   "PMC", 
   0xF1,0x04,0,0}
, {"L2_LINES_IN_ALL",
   "PMC", 
   0xF1,0x07,0,0}
, {"L2_LINES_OUT_DEMAND_CLEAN",
   "PMC", 
   0xF2,0x05,0,0}
, {"L2_LINES_OUT_DEMAND_DIRTY",
   "PMC", 
   0xF2,0x06,0,0}
, {"TX_MEM_ABORT_CONFLICT",
   "PMC", 
   0x54,0x01,0,0}
, {"TX_MEM_ABORT_CAPACITY",
   "PMC", 
   0x54,0x02,0,0}
, {"TX_MEM_ABORT_HLE_STORE_TO_ELIDED_LOCK",
   "PMC", 
   0x54,0x04,0,0}
, {"TX_MEM_ABORT_HLE_ELISION_BUFFER_NOT_EMPTY",
   "PMC", 
   0x54,0x08,0,0}
, {"TX_MEM_ABORT_HLE_ELISION_BUFFER_MISMATCH",
   "PMC", 
   0x54,0x10,0,0}
, {"TX_MEM_ABORT_HLE_ELISION_BUFFER_UNSUPP_ALIGNMENT",
   "PMC", 
   0x54,0x20,0,0}
, {"TX_MEM_ABORT_HLE_ELISION_BUFFER_FULL",
   "PMC", 
   0x54,0x40,0,0}
, {"TX_EXEC_MISC1",
   "PMC", 
   0x5D,0x01,0,0}
, {"TX_EXEC_MISC2",
   "PMC", 
   0x5D,0x02,0,0}
, {"TX_EXEC_MISC3",
   "PMC", 
   0x5D,0x04,0,0}
, {"TX_EXEC_MISC4",
   "PMC", 
   0x5D,0x08,0,0}
, {"HLE_RETIRED_START",
   "PMC", 
   0xC8,0x01,0,0}
, {"HLE_RETIRED_COMMIT",
   "PMC", 
   0xC8,0x02,0,0}
, {"HLE_RETIRED_ABORTED",
   "PMC", 
   0xC8,0x04,0,0}
, {"HLE_RETIRED_ABORTED_MISC1",
   "PMC", 
   0xC8,0x08,0,0}
, {"HLE_RETIRED_ABORTED_MISC2",
   "PMC", 
   0xC8,0x10,0,0}
, {"HLE_RETIRED_ABORTED_MISC3",
   "PMC", 
   0xC8,0x20,0,0}
, {"HLE_RETIRED_ABORTED_MISC4",
   "PMC", 
   0xC8,0x40,0,0}
, {"HLE_RETIRED_ABORTED_MISC5",
   "PMC", 
   0xC8,0x80,0,0}
, {"RTM_RETIRED_START",
   "PMC", 
   0xC9,0x01,0,0}
, {"RTM_RETIRED_COMMIT",
   "PMC", 
   0xC9,0x02,0,0}
, {"RTM_RETIRED_ABORTED",
   "PMC", 
   0xC9,0x04,0,0}
, {"RTM_RETIRED_ABORTED_MISC1",
   "PMC", 
   0xC9,0x08,0,0}
, {"RTM_RETIRED_ABORTED_MISC2",
   "PMC", 
   0xC9,0x10,0,0}
, {"RTM_RETIRED_ABORTED_MISC3",
   "PMC", 
   0xC9,0x20,0,0}
, {"RTM_RETIRED_ABORTED_MISC4",
   "PMC", 
   0xC9,0x40,0,0}
, {"RTM_RETIRED_ABORTED_MISC5",
   "PMC", 
   0xC9,0x80,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EXT_FILTER_MISS",
   "UPMC", 
   0x22,0x21,0,0}
, {"UNC_CBO_XSNP_RESPONSE_XCORE_FILTER_MISS",
   "UPMC", 
   0x22,0x41,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EVICTION_FILTER_MISS",
   "UPMC", 
   0x22,0x81,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EXT_FILTER_INVAL",
   "UPMC", 
   0x22,0x22,0,0}
, {"UNC_CBO_XSNP_RESPONSE_XCORE_FILTER_INVAL",
   "UPMC", 
   0x22,0x42,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EVICTION_FILTER_INVAL",
   "UPMC", 
   0x22,0x82,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EXT_FILTER_HIT",
   "UPMC", 
   0x22,0x24,0,0}
, {"UNC_CBO_XSNP_RESPONSE_XCORE_FILTER_HIT",
   "UPMC", 
   0x22,0x44,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EVICTION_FILTER_HIT",
   "UPMC", 
   0x22,0x84,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EXT_FILTER_HITM",
   "UPMC", 
   0x22,0x28,0,0}
, {"UNC_CBO_XSNP_RESPONSE_XCORE_FILTER_HITM",
   "UPMC", 
   0x22,0x48,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EVICTION_FILTER_HITM",
   "UPMC", 
   0x22,0x88,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EXT_FILTER_INVAL_M",
   "UPMC", 
   0x22,0x20,0,0}
, {"UNC_CBO_XSNP_RESPONSE_XCORE_FILTER_INVAL_M",
   "UPMC", 
   0x22,0x50,0,0}
, {"UNC_CBO_XSNP_RESPONSE_EVICTION_FILTER_INVAL_M",
   "UPMC", 
   0x22,0x90,0,0}
, {"UNC_CBO_CACHE_LOOKUP_READ_FILTER_M",
   "UPMC", 
   0x34,0x11,0,0}
, {"UNC_CBO_CACHE_LOOKUP_WRITE_FILTER_M",
   "UPMC", 
   0x34,0x26,0,0}
, {"UNC_CBO_CACHE_LOOKUP_EXTSNP_FILTER_M",
   "UPMC", 
   0x34,0x48,0,0}
, {"UNC_CBO_CACHE_LOOKUP_ANY_FILTER_M",
   "UPMC", 
   0x34,0x81,0,0}
, {"UNC_CBO_CACHE_LOOKUP_READ_FILTER_ES",
   "UPMC", 
   0x34,0x16,0,0}
, {"UNC_CBO_CACHE_LOOKUP_WRITE_FILTER_ES",
   "UPMC", 
   0x34,0x28,0,0}
, {"UNC_CBO_CACHE_LOOKUP_EXTSNP_FILTER_ES",
   "UPMC", 
   0x34,0x41,0,0}
, {"UNC_CBO_CACHE_LOOKUP_ANY_FILTER_ES",
   "UPMC", 
   0x34,0x86,0,0}
, {"UNC_CBO_CACHE_LOOKUP_READ_FILTER_I",
   "UPMC", 
   0x34,0x18,0,0}
, {"UNC_CBO_CACHE_LOOKUP_WRITE_FILTER_I",
   "UPMC", 
   0x34,0x21,0,0}
, {"UNC_CBO_CACHE_LOOKUP_EXTSNP_FILTER_I",
   "UPMC", 
   0x34,0x46,0,0}
, {"UNC_CBO_CACHE_LOOKUP_ANY_FILTER_I",
   "UPMC", 
   0x34,0x88,0,0}
, {"UNC_ARB_TRK_OCCUPANCY_ALL",
   "UPMC", 
   0x80,0x01,0,0}
, {"UNC_ARB_TRK_REQUEST_ALL",
   "UPMC", 
   0x81,0x01,0,0}
, {"UNC_ARB_TRK_REQUEST_WRITES",
   "UPMC", 
   0x81,0x20,0,0}
, {"UNC_ARB_TRK_REQUEST_EVICTIONS",
   "UPMC", 
   0x81,0x80,0,0}
, {"UNC_ARB_COH_TRK_OCCUPANCY_ALL",
   "UPMC", 
   0x83,0x01,0,0}
, {"UNC_ARB_COH_TRK_REQUEST_ALL",
   "UPMC", 
   0x84,0x01,0,0}
};
