...
#define NUM_GROUPS_PHI 18

static PerfmonGroupMap phi_group_map[NUM_GROUPS_PHI] = {

    {"CACHE",CACHE,0,"Compute to Data Access Ratio","VPU_ELEMENTS_ACTIVE:PMC0,DATA_READ_OR_WRITE:PMC1"},

    {"CPI",CPI,0,"Cycles per instruction","INSTRUCTIONS_EXECUTED:PMC0,CPU_CLK_UNHALTED:PMC1"},

    {"L2CACHE",L2CACHE,0,"L2 Compute to Data Access Ratio","VPU_ELEMENTS_ACTIVE:PMC0,DATA_READ_MISS_OR_WRITE_MISS:PMC1"},

    {"MEM1",MEM1,0,"L2 Write Misses","L2_DATA_WRITE_MISS_MEM_FILL:PMC0"},

    {"MEM2",MEM2,0,"L2 Read Misses","L2_DATA_READ_MISS_MEM_FILL:PMC0"},

    {"MEM3",MEM3,0,"HW prefetch transfers","HWP_L2MISS:PMC0"},

    {"MEM4",MEM4,0,"L2 Victim requests","L2_VICTIM_REQ_WITH_DATA:PMC0"},

    {"MEM5",MEM5,0,"L2 Snoop hits","SNP_HITM_L2:PMC0"},

    {"MEM6",MEM6,0,"L2 Read Misses","L2_READ_MISS:PMC0"},

    {"PAIRING",PAIRING,0,"Pairing ratio","INSTRUCTIONS_EXECUTED:PMC0,INSTRUCTIONS_EXECUTED_V_PIPE:PMC1"},

    {"READ_MISS_RATIO",READ_MISS_RATIO,0,"Miss ratio for data read","DATA_READ:PMC0,DATA_READ_MISS:PMC1"},

    {"VECTOR",VECTOR,0,"Vector unit usage","VPU_INSTRUCTIONS_EXECUTED:PMC0,VPU_ELEMENTS_ACTIVE:PMC1"},

    {"VECTOR2",VECTOR2,0,"Vector unit usage","VPU_INSTRUCTIONS_EXECUTED:PMC0,VPU_STALL_REG:PMC1"},

    {"VPU_FILL_RATIO_DBL",VPU_FILL_RATIO_DBL,0,"VPU filling for Double","VPU_INSTRUCTIONS_EXECUTED:PMC0,VPU_ELEMENTS_ACTIVE:PMC1"},

    {"VPU_PAIRING",VPU_PAIRING,0,"VPU Pairing ratio","VPU_INSTRUCTIONS_EXECUTED:PMC0,VPU_INSTRUCTIONS_EXECUTED_V_PIPE:PMC1"},

    {"VPU_READ_MISS_RATIO",VPU_READ_MISS_RATIO,0,"Miss ratio for VPU data read","VPU_DATA_READ:PMC0,VPU_DATA_READ_MISS:PMC1"},

    {"VPU_WRITE_MISS_RATIO",VPU_WRITE_MISS_RATIO,0,"Miss ratio for VPU data write","VPU_DATA_WRITE:PMC0,VPU_DATA_WRITE_MISS:PMC1"},

    {"WRITE_MISS_RATIO",WRITE_MISS_RATIO,0,"Miss ratio for data write","DATA_WRITE:PMC0,DATA_WRITE_MISS:PMC1"},
...
perfmon_printDerivedMetricsPhi(PerfmonGroup group)
{
    ...
    switch ( group ) 
    {
        ...
        case MEM1:
            ...
            bstrListAdd(fc,2,RFO Data Bandwidth [MBytes/s]);

            bstrListAdd(fc,3,RFO Volume [GBytes]);
        ...
        case MEM2:
            ...
            bstrListAdd(fc,3,Read Data Volume [GBytes]);
        ...
        case MEM3:
            ...
            bstrListAdd(fc,2,Prefetch Data Bandwidth [MBytes/s]);

            bstrListAdd(fc,3,Prefetch Data Volume [GBytes]);
        ...
        case MEM4:
            ...
            bstrListAdd(fc,2,Victim Data Bandwidth [MBytes/s]);

            bstrListAdd(fc,3,Victim Data Volume [GBytes]);
        ...
        case MEM5:
            ...
            bstrListAdd(fc,2,Snoop Data Bandwidth [MBytes/s]);

            bstrListAdd(fc,3,Snoop Data Volume [GBytes]);
        ...
        case MEM6:
            ...
            bstrListAdd(fc,2,L2 Read Data Bandwidth [MBytes/s]);

            bstrListAdd(fc,3,L2 Read Data Volume [GBytes]);
...
perfmon_logDerivedMetricsPhi(PerfmonGroup group, double time,double timeStamp)
{
    ...
    switch ( group ) 
    {
        ...
        case MEM1:
                        ...
                        printf("RFO Data Bandwidth [MBytes/s] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-06*perfmon_getResult(threadId,"PMC0")*64.0/time;
                            if (!isnan(tmpValue))
                        ...
                        printf("RFO Volume [GBytes] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-09*perfmon_getResult(threadId,"PMC0")*64.0;
                            if (!isnan(tmpValue))
        ...
        case MEM2:
                        ...
                        printf("Read Data Volume [GBytes] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-09*perfmon_getResult(threadId,"PMC0")*64.0;
                            if (!isnan(tmpValue))
        ...
        case MEM3:
                        ...
                        printf("Prefetch Data Bandwidth [MBytes/s] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-06*perfmon_getResult(threadId,"PMC0")*64.0/time;
                            if (!isnan(tmpValue))
                        ...
                        printf("Prefetch Data Volume [GBytes] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-09*perfmon_getResult(threadId,"PMC0")*64.0;
                            if (!isnan(tmpValue))
        ...
        case MEM4:
                        ...
                        printf("Victim Data Bandwidth [MBytes/s] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-06*perfmon_getResult(threadId,"PMC0")*64.0/time;
                            if (!isnan(tmpValue))
                        ...
                        printf("Victim Data Volume [GBytes] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-09*perfmon_getResult(threadId,"PMC0")*64.0;
                            if (!isnan(tmpValue))
        ...
        case MEM5:
                        ...
                        printf("Snoop Data Bandwidth [MBytes/s] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-06*perfmon_getResult(threadId,"PMC0")*64.0/time;
                            if (!isnan(tmpValue))
                        ...
                        printf("Snoop Data Volume [GBytes] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-09*perfmon_getResult(threadId,"PMC0")*64.0;
                            if (!isnan(tmpValue))
        ...
        case MEM6:
                        ...
                        printf("L2 Read Data Bandwidth [MBytes/s] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-06*perfmon_getResult(threadId,"PMC0")*64.0/time;
                            if (!isnan(tmpValue))
                        ...
                        printf("L2 Read Data Volume [GBytes] %e ",timeStamp);
                        for(threadId=0; threadId < perfmon_numThreads; threadId++)
                        {
                            tmpValue = 1.0E-09*perfmon_getResult(threadId,"PMC0")*64.0;
                            if (!isnan(tmpValue))
